...
const AdminPersetujuan = () => {
    ...
    const getData = async()=>{
        try {
            ...
            setPendingProgram(response.data.filter((item)=>item.verifikasi == 'pending'))
    ...
    const updateStatusSetuju = async()=>{
        try {
            const response = await axios.patch(`http://localhost:3000/program/${selectedProgram}`,{
    ...
    <>
        <div className="flex flex-col justify-center items-center w-full border rounded-xl shadow-xl p-10 bg-white">
            <div className="w-full flex flex-col justify-start items-start gap-10 mb-5">
               <div className="w-full flex flex-col justify-start items-start gap-2">
                  ...
                  <p className='text-gray-500'>Review and approve or reject pending program submissions</p>
               </div>

               <div className='flex flex-wrap justify-center items-center gap-10 w-full'>
                {pendingProgram.length === 0 ? <p className='font-semibold text-lg'>Tidak ada data program pending</p> : (
                     pendingProgram.map((value)=>{
                        return(
                            <div className='w-full border rounded-lg shadow-md text-xs ' key={value.id}>
                                <div className='p-5 bg-gradient-to-tr from-amber-50 to-white flex justify-between items-start'>
                                    <div className='w-full text-sm'>
                                        <h2 className='text-base font-semibold uppercase'>{value.program} - {value.nama_desa} </h2>
                                        <p>{value.jenis_kegiatan} </p>
                                        <p className='font-light text-[11px]'>Submit by :{value.user.name} </p>
                                    </div>
                                ...
                                <div className='flex flex-col justify-start items-start'>
                                    <div className='w-full p-5'>
                                        ...
                                        <p className='text-xs'>{value.keterangan} </p>
                                    </div>
                                    <div className='flex justify-around items-center w-full gap-5 p-5 '>
                                        ...
                                        <div className='w-full p-5 border rounded-md flex justify-start items-center gap-3'>
                                            ...
                                            <div>
                                                <p>Budget</p>
                                                <p>
                                                {value.alokasi_dana?.toLocaleString('id-ID', {
                                                    style: 'currency',
                                                    currency: 'IDR',
                                                })}
                                                </p>
                                        ...
                                        <div className='w-full flex flex-col justify-between gap-2 items-center  '>
                                            <button className='flex justify-center items-center gap-3 h-full w-full rounded-md bg-green-700 text-white p-2' onClick={()=>handleClickSetuju(value.id)}>
                                                <CheckCircleOutlineTwoToneIcon sx={{fontSize :'15px'}}/>
                                                <p>Setujui</p>
                                            </button>
                                            <button className='flex justify-center items-center gap-3 h-full w-full rounded-md border border-red-500 text-red-500 p-2' onClick={()=>handleClickTolak(value.id)} >
                                                <CloseTwoToneIcon sx={{fontSize :'15px'}} />
                                                <p>Tolak</p>
                                            </button>
                                        </div>
        ...
        >
            ...
            <DialogContentText id="alert-dialog-description">
                Konfirmasi persetujuan program untuk approve program yang telah diajukan
            </DialogContentText>
            </DialogContent>
            <DialogActions>
